...
_RAW_ETF_FACTOR_LIBRARY: list[dict] = [
    ...
    },
    {
        ...
        "description": "当日成交量相对过去 5 日均值的放大程度",
        ...
        "supported_backends": ["local", "bigquant"],
    ...
    {
        "name": "bbi_indicator",
        "description": "多空指标 BBI = (MA3 + MA6 + MA12 + MA24) / 4，用于判断多空分界",
        "formula": "(m_avg(close, 3) + m_avg(close, 6) + m_avg(close, 12) + m_avg(close, 24)) / 4",
        "direction": "neutral",
        "category": "trend",
        "factor_type": "趋势因子",
        "source_ideas": ["BBI 多空指标", "MA 组合"],
        "bigquant_ready": True,
        "pandas_hint": "在 pandas 中先按 instrument groupby 再 rolling 3/6/12/24 求均值",
        "mvp_default": False,
        "uses_future_data": False,
        "implementation_status": "implemented",
        "supported_backends": ["local", "bigquant"],
